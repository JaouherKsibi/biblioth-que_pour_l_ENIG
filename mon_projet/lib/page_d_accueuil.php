...
session_start();
    ...
    if(isset($ok))
    {
        try{
                ...
                $req=$dbco->prepare("SELECT nom,prenom,niveau,specialite FROM etudiant_enig WHERE adresseENIG='$adresse_enig'");
                $execute_is_ok=$req->execute(array('nom','prenom','niveau','specialite'));
                if ($execute_is_ok==false) {
                    ...
                    if ($user_exist==1) {
                        $userinfo=$req->fetch();
                        $nom_etudiant=$userinfo['nom'];
                        $prenom_etudiant=$userinfo['prenom'];
                        $niveau_etudiant=$userinfo['niveau'];
                        $specialite_etudiant=$userinfo['specialite'];
                        $connection=true;
                    }
                    else {
    ...
    function afficher_document($id_doc,$nom_doc,$matiere,$nom_complet_professeur,$niveau_scolaire,$type_doc,$lien_doc,$specialite)
    {
        ...
        echo "
                            <article class='col-md-4 col-lg-4 col-xs-12 col-sm-12 document'>
                                <div class='container'>
                                    ...
                                    <p><em>specialite:</em> $specialite</p>
...
<head>
    ...
    <title>
        acueuil <?php echo "$nom_etudiant $prenom_etudiant";?>
    </title>
</head>
<body>
    <header>
     <div class="wrapper">
         ...
         <ul class="nav-area">
             ...
             <li>
                 <p><img src="../img/logoENIG1.jpeg" alt=" user image"><?php echo "$nom_etudiant $prenom_etudiant";?></p>
        ...
        <div class="container">
            ...
            <?php
                try{
                    ...
                    $req_doc=$dbco_doc->prepare("SELECT id_doc,nom_doc,matiere,nom_complet_professeur,niveau_scolaire,type_doc,lien_doc,specialite FROM document WHERE niveau_scolaire=$niveau_etudiant AND specialite='$specialite_etudiant' AND type_doc='cours'");
        ...
        <div class="container">
            ...
            <?php
                try{
                    ...
                    $dbco_doc = new PDO("mysql:host=$servername;dbname=$dbname", $username, $password);
                    $dbco_doc->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
                    $req_doc=$dbco_doc->prepare("SELECT id_doc,nom_doc,matiere,nom_complet_professeur,niveau_scolaire,type_doc,lien_doc,specialite FROM document WHERE niveau_scolaire=$niveau_etudiant AND specialite='$specialite_etudiant' AND type_doc='serie d exercice'");
                    $execute_is_ok=$req_doc->execute(array('id_doc','nom_doc','matiere','nom_complet_professeur','niveau_scolaire','type_doc','lien_doc','specialite'));
                        if ($execute_is_ok==false) {
                            $erreur='la connection a la base de données et la récupération des documents a échoué!';
                        }
        ...
        <div class="container">
            ...
            <?php
                try{
                    ...
                    $dbco_doc = new PDO("mysql:host=$servername;dbname=$dbname", $username, $password);
                    $dbco_doc->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
                    $req_doc=$dbco_doc->prepare("SELECT id_doc,nom_doc,matiere,nom_complet_professeur,niveau_scolaire,type_doc,lien_doc,specialite FROM document WHERE niveau_scolaire=$niveau_etudiant AND specialite='$specialite_etudiant' AND type_doc='examen'");
                    $execute_is_ok=$req_doc->execute(array('id_doc','nom_doc','matiere','nom_complet_professeur','niveau_scolaire','type_doc','lien_doc','specialite'));
                        if ($execute_is_ok==false) {
                            $erreur='la connection a la base de données et la récupération des documents a échoué!';
                        }
